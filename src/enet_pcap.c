...
static void pcap_tick(void)
{
    ...
    if (pcap_started) {
        ...
        if (data && h.caplen > 0) {
            ...
            QueueEnter(pcapq,p);
            SDL_UnlockMutex(pcap_mutex);
            Log_Printf(LOG_INFO, "[PCAP] Output packet with %i bytes to queue",h.caplen);
        }
    }
}
...
void enet_pcap_queue_poll(void)
{
    if (pcap_started) {
        ...
        if (QueuePeek(pcapq)>0)
        {
            ...
            Log_Printf(LOG_INFO, "[PCAP] Getting packet from queue");
...
void enet_pcap_input(Uint8 *pkt, int pkt_len) {
    if (pcap_started) {
        Log_Printf(LOG_INFO, "[PCAP] Input packet with %i bytes",enet_tx_buffer.size);
        SDL_LockMutex(pcap_mutex);
