...
void slirp_output (const unsigned char *pkt, int pkt_len)
{
    ...
    Log_Printf(LOG_INFO, "[SLIRP] Output packet with %i bytes to queue",pkt_len);
}
...
void enet_slirp_queue_poll(void)
{
    ...
    if (QueuePeek(slirpq)>0)
    {
        ...
        Log_Printf(LOG_INFO, "[SLIRP] Getting packet from queue");
...
void enet_slirp_input(Uint8 *pkt, int pkt_len) {
    if (slirp_started) {
        Log_Printf(LOG_INFO, "[SLIRP] Input packet with %i bytes",enet_tx_buffer.size);
        SDL_LockMutex(slirp_mutex);
